...
def portfolio(
    ...
    reserch = filter_onePeriod(
        dataframe=filter_csv,
    ...
    engine = backtester_engine(
        overall=reserch,
        dataset=dataset,
        start_date=start_date,
        end_date=end_date,
        cash=cash,
        transactions_cost=slippage / 100,
        max_weight=max_weight,
        min_weight=min_weight,
    )
    engine.run()
    # get portfolio_rets
    portfolio_values = np.array(engine.cash_series) + np.array(
        engine.portfolio_stocks_value_series
    )
    portfolio_rets = pd.Series(
        portfolio_values,
        index=[datetime.datetime.strptime(i, "%Y-%m-%d") for i in engine.timeline],
    ).pct_change(1)
    ...
    qs.reports.html(
        portfolio_rets,
        benchmark=benchmark,
        output=os.path.join("reports", report_name),
        title="Crypto Strategy Tearsheet",
    )
...
if __name__ == "__main__":
    ...
    for i in range(9):
        ...
        cur_log, cur_research = portfolio(
            ...
            price_upper_limit=price_upper_limit[i],
            short_term_ret_lower=short_term_ret_lower[i],
            short_term_ret_upper=short_term_ret_upper[i],
            ...
            dollar_vol_ratio_upper=dollar_vol_ratio_upper[i],
            range_test=range_test[i],
            dropdown_allow_in_gain=dropdown_allow_in_gain[i],
            stop_loss_test_period=stop_loss_test_period[i],
            stop_gain_test_period=stop_gain_test_period[i],
            cash=cash[i],
            slippage=slippage[i],
